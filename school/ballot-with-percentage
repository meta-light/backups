...
contract Ballot {
   ...
   uint public finalPercentage = 0;
  
   mapping(uint => vote) private votes;
   mapping(address => voter) public voterRegister;
  
   enum State { Created, Voting, Ended, Report }
   State public state;
   ...
   function ballotOfficialProxy() 
       private 
       inState(State.Voting)
       onlyOfficial 
    {
       if (ballotOfficialAddress = voterAddress ) {
            address addr = address(keccak256(abi.encodePacked(now)));
            doVote(msg.sender.vote)
       }
       ...
       onlyOfficial
   {
    ...
    function reportResult()
       public
       inState(State.Report)
       onlyOfficial
   {
       state = State.Ended;
       finalResult = countResult; //move result from private countResult to public finalResult
       finalPercentage = totalVote/finalResult;
       emit voteEnded(finalResult);
   }
